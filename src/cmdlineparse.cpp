...
user_options_t parseCommandLineOptions( int ac, char* av[] )
{
  ...
  desc.add_options( )
    ...
    ( "aero-f,e",   "output in aero-f mode instead of the default aero-s mode. Surface type "
          "(StickFixedXXX_i, SlipMovingXXX_i, InletFixedXXX_i, etc.) may be defined by using the "
          "-n [ --names ] argument." )

    ( "matusage,m", "outputs matusage directives in addition to attributes. To be used when relevant matlaw is enabled." )
    ...
    ( "verbose,v", "verbose mode. Messages are streamed to std::clog (and hence stderr)." );
  ...
  if ( vm.count( "help" ) )
  {
    ...
    cout << '\n';
    cout << "This software uses open source software packages from third parties. These are available on\n";
    cout << "an \"as is\" basis and subject to their individual license agreements. Additional information can\n";
